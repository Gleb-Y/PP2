...
#e for eraser
#s for square
#t for right_triangle
#l for equilateral_triangle
#h for rombus
#space отчистить экран
        ...
        for event in events:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_r:
                    ...
                    # Switch to square drawing mode
                    self.current_shape = 'square'
                elif event.key == pygame.K_t:
                    # Switch to right triangle drawing mode
                    self.current_shape = 'right_triangle'
                elif event.key == pygame.K_l:
                    # Switch to equilateral triangle drawing mode
                    self.current_shape = 'equilateral_triangle'
                elif event.key == pygame.K_h:
                    # Switch to rhombus drawing mode
                ...
                if event.button == 1:  # Left mouse button
                    ...
                    if self.current_shape == 'rectangle':
                        ...
                        self.shapes.append({'type': 'square', 'start': start_pos, 'end': start_pos, 'color': self.current_color})
                    elif self.current_shape == 'right_triangle':
                        self.shapes.append({'type': 'right_triangle', 'start': start_pos, 'end': start_pos, 'color': self.current_color})
                    elif self.current_shape == 'equilateral_triangle':
                        self.shapes.append({'type': 'equilateral_triangle', 'start': start_pos, 'end': start_pos, 'color': self.current_color})
                    elif self.current_shape == 'rhombus':
                        self.shapes.append({'type': 'rhombus', 'start': start_pos, 'end': start_pos, 'color': self.current_color})
    ...
    def Render(self, screen):
        ...
        for shape in self.shapes:
            if shape['type'] == 'rectangle':
                ...
                pygame.draw.rect(screen, shape['color'], pygame.Rect(shape['start'], (side_length, side_length)))
            elif shape['type'] == 'right_triangle':
                pygame.draw.polygon(screen, shape['color'], [shape['start'], (shape['end'][0], shape['start'][1]), shape['end']])
            elif shape['type'] == 'equilateral_triangle':
                width = abs(shape['end'][0] - shape['start'][0])
                height = abs(shape['end'][1] - shape['start'][1])
                side_length = min(width, height)
                half_side = side_length / 2
                pygame.draw.polygon(screen, shape['color'], [shape['start'], (shape['start'][0] + side_length, shape['start'][1]), (shape['start'][0] + half_side, shape['start'][1] - (side_length * (3 ** 0.5) / 2))])
            elif shape['type'] == 'rhombus':
                pygame.draw.polygon(screen, shape['color'], [(shape['start'][0], shape['start'][1] - 50), (shape['start'][0] + 50, shape['start'][1]), (shape['start'][0], shape['start'][1] + 50), (shape['start'][0] - 50, shape['start'][1])])
...
# Start the game by creating an instance of GameScene and running the game loop
